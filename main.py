...
def load_configuration_settings(source_id, source_name, **kwargs):
    global sources_list, polygons, loaded_camera_ids, max_time_threshold, allowed_People
    try:
...
class DataProcessor:
    ...
    def process_data(self, data, **kwargs):
        ...
        # logger.debug(detected_object_list)       
        # if len(detected_object_list) > 2:
        if len(detected_object_list) > allowed_People:
            post_process(
                connector=connector,
